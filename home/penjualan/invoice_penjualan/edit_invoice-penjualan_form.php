                                ...
                                <div>
                                    <table id="detail-invoice-penjualan" class="table small table-small table-bordered table-hover table-striped" style="width: 1000px;">
                                        ...
                                        <tbody>
                                            ...
                                            $queryDetailInvoicePenjualan = mysqli_query($conn, "SELECT A.urutan AS no, A.no_transaksi, D.no_transaksi, A.kode_barang, E.nama AS nama_barang, IFNULL(D.keterangan, '') AS keterangan, ( F.qty_SO -( F.qty_terjual - IFNULL(D.quantity, 0) ) ) AS quantity, IFNULL(D.quantity, 0) AS quantity_terjual, A.harga, A.diskon_persentase, A.diskon FROM `detail_sales_order` AS A LEFT JOIN sales_order AS B ON B.no_transaksi = A.no_transaksi LEFT JOIN invoice_penjualan AS C ON C.no_so = B.no_transaksi LEFT JOIN detail_invoice_penjualan AS D ON D.no_transaksi = C.no_transaksi AND D.kode_barang = A.kode_barang LEFT JOIN barang AS E ON E.id_barang = A.kode_barang LEFT JOIN so_invoice AS F ON F.no_transaksi = A.no_transaksi AND F.kode_barang = A.kode_barang where A.no_transaksi = '$noSO' ORDER BY NO ASC;");
