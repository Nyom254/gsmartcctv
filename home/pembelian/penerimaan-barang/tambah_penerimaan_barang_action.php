<?php
    ...
    $queryTambahPenerimaan = mysqli_prepare($conn,"insert into penerimaan (no_penerimaan, tgl, no_po, kode_supplier, keterangan) values (?, ?, ?, ?, ?)");
    mysqli_stmt_bind_param($queryTambahPenerimaan, "sssss", $no_penerimaan, $tanggal, $no_po, $kode_supplier, $keterangan);
        ...
        if(mysqli_stmt_execute($queryTambahPenerimaan)){
            for($i = 0; $i < count($_POST['kode-barang']); $i++){
                ...
                if(isset($quantity[$i]) && $quantity[$i] != '' && $quantity[$i] != 0) {
                    $queryUpdateQtyStok = mysqli_prepare($conn, "update saldo_stok set qty = qty + ? where no_transaksi = ? and kode_barang = ?");
                    mysqli_stmt_bind_param($queryUpdateQtyStok, "iss", $quantity[$i], $no_po, $kode_barang[$i]);
                    mysqli_stmt_execute($queryUpdateQtyStok);
                    
                    
                    if(mysqli_stmt_affected_rows($queryUpdateQtyStok) == 0){
                        mysqli_stmt_close($queryUpdateQtyStok);
